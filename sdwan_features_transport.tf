...
resource "sdwan_transport_wan_vpn_feature_associate_routing_bgp_feature" "transport_wan_vpn_feature_associate_routing_bgp_feature" {
  for_each = {
    ...
    : "${bgp_association.profile.name}-${bgp_association.association.transport_wan_vpn_feature}-${bgp_association.association.transport_routing_bgp_feature}" => bgp_association
  }
  feature_profile_id               = sdwan_transport_feature_profile.transport_feature_profile[each.value.profile.name].id
  transport_wan_vpn_feature_id     = try(sdwan_transport_wan_vpn_feature.transport_wan_vpn_feature["${each.value.profile.name}-wan_vpn"].id, null)
  transport_routing_bgp_feature_id = try(sdwan_transport_routing_bgp_feature.transport_routing_bgp_feature["${each.value.profile.name}-${each.value.association.transport_routing_bgp_feature}"].id, null)
}
...
resource "sdwan_transport_routing_bgp_feature" "transport_routing_bgp_feature" {
  ...
  ipv4_neighbors           = try(length(each.value.bgp.ipv4_neighbors) == 0, true) ? null : [for ipv4_neighbor in each.value.bgp.ipv4_neighbors : {
    ...
    address_families = try(length(ipv4_neighbor.address_families) == 0, true) ? null : [for ipv4_neighbor_af in ipv4_neighbor.address_families : {
      ...
      in_route_policy_id                = try(sdwan_transport_route_policy_feature.transport_route_policy_feature["${each.value.profile.name}-${ipv4_neighbor_af.in_route_policy}"].id, null)
      ...
      out_route_policy_id               = try(sdwan_transport_route_policy_feature.transport_route_policy_feature["${each.value.profile.name}-${ipv4_neighbor_af.out_route_policy}"].id, null)
  ...
  ipv4_redistributes = try(length(each.value.bgp.ipv4_redistributes) == 0, true) ? null : [for ipv4_redist in each.value.bgp.ipv4_redistributes : {
    ...
    route_policy_id        = try(sdwan_transport_route_policy_feature.transport_route_policy_feature["${each.value.profile.name}-${ipv4_redist.route_policy}"].id, null)
  }]
