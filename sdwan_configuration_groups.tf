resource "sdwan_configuration_group" "configuration_group" {
  ...
  topology_devices = try(length(each.value.topology_devices) == 0, true) ? null : [for t in each.value.topology_devices : {
    ...
    unsupported_features = try(length(t.unsupported_features) == 0, true) ? null : [for u in t.unsupported_features : (
      ...
      u.parcel_type == "route-policy" ? ({
        parcel_type = try(u.parcel_type)
        parcel_id   = try(sdwan_service_route_policy_feature.route_policy_feature["${u.parcel_profile}-${u.parcel_name}"].id, null)
      }) :
      null
    )]
  }]
  devices = try(length(each.value.devices) == 0, true) ? null : [for d in each.value.devices : {
    id     = try(d.id, null)
    deploy = try(d.deploy, null)
    variables = try(length(d.variables) == 0, true) ? null : [for v in d.variables : {
      name  = try(v.name, null)
      value = try(v.value, null)
    }]
  }]
}